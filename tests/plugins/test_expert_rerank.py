...
class _FixtureStore(MockKnowledgeStorePort):
    ...
    async def query(
        self, collection: str, query_texts: list[str], n_results: int = 10, where=None,
    ) -> QueryResult:
...
class TestLexicalMatchesSurviveReranking:
    async def test_a_none_distance_does_not_crash_the_rerank(self) -> None:
        """#114 gives a literally-matched passage distance None; 1.0 - None
        raised TypeError inside score prep, failing the whole request."""
        from core.domain.rerank import LexicalReranker

        class _MixedStore:
            async def query(self, collection, query_texts, n_results=10, where=None):
                return QueryResult(
                    documents=[["semantic passage", "literal passage"]],
        ...
        plugin = ExpertPlugin(
            llm=MockLLMPort(response="an answer"),
            knowledge_store=_MixedStore(),
            reranker=LexicalReranker(),
            score_threshold=0.0,
        )
        response = await plugin.handle(make_input(message="literal passage?"))
        assert response.result == "an answer"
