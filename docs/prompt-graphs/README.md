...
|---|---|---|
| `collection_template` | yes | may reference **only** `{bok_id}`, unmodified — the engine-seeded body-of-knowledge id; any other variable, or a format spec/conversion on `{bok_id}` itself (e.g. `{bok_id:.0}`, `{bok_id!r}`), is a configuration error at parse time (tenancy boundary, not a formatting concern). A positional field — a bare `{}` and an explicit index (`{0}`) — is likewise rejected at parse time; only named `{variable}` references are supported, and each such name must be a valid Python identifier (`str.isidentifier()`). |
| `query_template` | yes | filled with state values, single pass. A positional field — a bare `{}` and an explicit index (`{0}`) — is rejected at parse time, same as `collection_template`. Template variables must be valid Python identifiers (`str.isidentifier()`); a named but non-identifier state key (e.g. `user-name`) cannot be referenced from a template even though the state model itself accepts it. |
