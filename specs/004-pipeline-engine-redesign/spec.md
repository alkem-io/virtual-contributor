...
- **FR-011**: The StoreStep MUST process chunks in configurable batches (default: 50), calling the knowledge store once per batch with pre-computed embeddings. When an EmbedStep has run (some chunks have embeddings), StoreStep MUST skip chunks without embeddings to prevent storing entries with mismatched embedding models. When no chunks have embeddings (no EmbedStep in pipeline), StoreStep stores all chunks and delegates embedding to the knowledge store.
