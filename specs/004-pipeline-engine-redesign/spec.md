...
- **FR-007**: Summarization MUST use progressive length budgeting with a target length of 10000 characters (matching original repo) — early chunks receive ~40% of the target (4000 chars), scaling linearly to 100% (10000 chars) for later chunks. When there is only a single section/chunk to summarize, it receives the full 100% budget.
